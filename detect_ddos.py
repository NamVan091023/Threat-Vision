...
Original file is located at
    https://colab.research.google.com/drive/187gDcDKJMYnTI6BOu2vmFy4cazkc9oX9
"""
...
#Thu thập và lưu đường dẫn

dfps_train = []
dfps_test = []

for dirname, _, filenames in os.walk('/content/drive/MyDrive/ThreatVision/CIC_2019 ML'):
...
# đọc và kết hợp các file train và test

train_df = pd.concat([pd.read_parquet(dfp) for dfp in tqdm(dfps_train)], ignore_index=True)
test_df = pd.concat([pd.read_parquet(dfp) for dfp in tqdm(dfps_test)], ignore_index=True)
...
print(train_df.isnull().sum().sum())  # Kiểm tra tổng số giá trị NaN
...
print(f"{train_df.isnull().sum().sum()}")
...
def cat_summary(data, col, plot=False):
    ...
    if plot:
        ...
        plt.title(f"Tần suất xuất hiện  {col}")
...
plt.title("Phân phối thời lượng luồng cho DDos so với lưu lượng bình thường")
plt.show()
...
print(f"Tổng số cột có tương quan cao: {len(high_corr_cols)}")
print("Các cột có tương quan cao là:", high_corr_cols)
...
# Gộp tất cả nhãn lại trước khi fit
all_labels = list(y_train) + list(y_val) + list(y_test)

le = LabelEncoder()
le.fit(all_labels)  # Fit trên toàn bộ dữ liệu nhãn
...
def train_model(X_train, X_test, y_train, y_test):
    ...
    "XGBoost": XGBClassifier(tree_method='gpu_hist'),  # Sử dụng GPU
}
